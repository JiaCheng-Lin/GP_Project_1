...
public class TopCheckGate : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(canEnter && Input.GetKeyDown("s")) // just do once
        {
            ...
            StartCoroutine(Enter(player.transform));
        ...
        if(startTrans)
        {
            if (player.transform.position.x <= targetP.position.x-2 && player.transform.position.x >= targetP.position.x-3)
                player.transform.localScale = Vector3.one * 1.1f;

            if (player.transform.position.y >= targetP.position.y) { 
                //player.transform.localScale = Vector3.one/2;
                player.transform.position += Vector3.down * vertical_speed * Time.deltaTime;
            }
            else if(player.transform.position.x <= targetP.position.x+1) 
                player.transform.position += Vector3.right * hor_speed * Time.deltaTime;
            else
            {
                //player.transform.localScale = Vector3.one * 1.1f;
                player.GetComponent<PlayerMovement>().enabled = true;
    ...
    {
        Vector3 enteredPosition = startP.position + Vector3.down;
